...
// DA « Nuit » : piste verte (accent-dark) quand actif, gris nuit sinon ;
// pastille blanche qui glisse en .2s.
...
export function Toggle({ on, onToggle, label }: ToggleProps) {
  return (
    <button
      ...
      aria-label={label}
      onClick={onToggle}
      className={`relative h-[21px] w-9 flex-none cursor-pointer rounded-full transition-colors duration-200 ${on ? "bg-accent-dark" : "bg-night-border"}`}
    >
      <span
